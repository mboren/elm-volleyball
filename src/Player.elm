...
updateLegs player =
    let
        ...
        wallToWallDistance =
            player.rightWallX - player.leftWallX

        numStridesFromWallToWall =
            6

        strideLength =
            wallToWallDistance / numStridesFromWallToWall
...
updateArms target player =
    let
        newLeft =
            updateHandPosition target player player.leftArm

        newRight =
            updateHandPosition target player player.rightArm
    in
    { player
        | leftArm = newLeft
        , rightArm = newRight
...
updateHandPosition target player arm =
    let
        ...
        distance =
            V2.length restingPositionToTarget
        ...
        wallCompensatedPosition wallX =
            let
                dx =
                    wallX - V2.getX absoluteShoulderPosition

                radical =
                    max 0 (arm.length * arm.length - dx * dx)
        ...
        wallCompensatedHandPosition =
            ...
            else if absoluteHandX >= player.rightWallX then
